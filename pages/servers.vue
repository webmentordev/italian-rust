    ...
    <section class="w-full bg-center bg-cover min-h-screen relative" style="background-image: url('/servers.jpg');">
        ...
        <div class="relative z-10 max-w-6xl m-auto pt-[140px] px-4">
            ...
            <div v-else class="flex flex-col p-3 max-w-lg m-auto rounded-lg mt-6 bg-dark relative">
                ...
                <img :src="server.details.rust_headerimage" class="rounded-lg" alt="Italian Rust Header Image">
                <div class="p-3">
                    ...
                    <div class="m-auto 510px:text-start 510px:m-0 w-fit flex 510px:flex-col text-white border-l-main 510px:border-l">
                        <div class="flex items-center 510px:border-none text-end 510px:text-start py-2 px-3">
                            <h3 class="border-r border-main pr-3 mr-3">Last Wipe</h3>
                ...
                <NuxtLink :to='`steam://connect/${server.ip}:${server.portQuery}`' class="link bg-main w-full text-center py-3 text-lg rounded-lg hover:text-white hover:bg-black transition-all" target="_blank" rel="nofollow">Connect</NuxtLink>
            </div>
        </div>
    </section>
</template>
...
<script setup>
    ...
    onMounted(() => {
        $fetch("https://api.battlemetrics.com/servers/25062240").then(result => {
